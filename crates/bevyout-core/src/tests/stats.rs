...
fn derived_attributes_match_the_gmst_formula() {
    ...
    assert_eq!(derived.max_health, 210.0);
    assert_eq!(derived.max_action_points, 80.0);
...
fn derived_attributes_grow_with_level_and_special() {
    ...
    let derived = world.derived(&settings());
    assert_eq!(derived.max_health, 300.0);
...
fn gmst_settings_override_from_pairs_and_skip_bad_values() {
    let pairs = vec![
        ...
        ("IXPBASE", GmstValue::Int(200)),
        (
            GMST_CARRY_WEIGHT_BASE,
    ...
    assert_eq!(overridden.xp_base, 200);
...
fn gmst_settings_validation_rejects_non_finite_and_out_of_range() {
    ...
    negative.carry_weight_strength_mult = -1.0;
    assert!(matches!(
        negative.validate(),
        Err(StatsError::NegativeSetting("carry_weight_strength_mult"))
    ));
...
fn xp_thresholds_follow_the_quadratic_curve() {
    ...
    assert_eq!(xp_threshold(2, &settings), 150);
    assert_eq!(xp_threshold(3, &settings), 450);
    assert_eq!(xp_threshold(4, &settings), 900);
    assert_eq!(xp_threshold(30, &settings), 65_250);
}

#[test]
fn awarding_threshold_xp_levels_up_once_with_skill_points() {
    let mut world = sheet();
    let outcome = award_xp(&mut world, 150, &settings());
    assert_eq!(outcome.levels_gained, 1);
    assert_eq!(outcome.level, 2);
    assert_eq!(outcome.xp, 150);
    assert_eq!(world.level, 2);
    assert_eq!(world.xp_into_level(&settings()), 0);
    // 10 base + intelligence 5.
...
fn one_award_can_cross_several_thresholds() {
    let mut world = sheet();
    let outcome = award_xp(&mut world, 500, &settings());
    assert_eq!(outcome.level, 3);
    assert_eq!(outcome.levels_gained, 2);
    assert_eq!(world.xp_into_level(&settings()), 50);
...
fn character_sheet_round_trips_through_serde() {
    ...
    award_xp(&mut world, 500, &settings());
...
fn default_sheet_and_settings_are_the_documented_goty_values() {
    ...
    assert_eq!(defaults.action_points_agility_mult, 3.0);
    assert_eq!(defaults.carry_weight_base, 150.0);
    assert_eq!(defaults.carry_weight_strength_mult, 10.0);
    assert_eq!(defaults.max_player_level, 30);
    assert_eq!(defaults.level_up_skill_points, 10);
    assert_eq!(defaults.xp_base, 150);
    let world = sheet();
    for attribute in [
