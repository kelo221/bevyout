...
fn spot_light(
    ...
    if (*light).decal_index != 0xFFFFFFFFu {
        ...
        if local_position.z < 0.0 {
            let spot_light_tan_angle = bitcast<f32>((*light).shadow_map_index_or_spot_light_tan_angle);
            let decal_uv = (local_position.xy / (local_position.z * spot_light_tan_angle)) * vec2(-0.5, 0.5) + 0.5;
            let image_index = view_bindings::clustered_decals.decals[(*light).decal_index].base_color_texture_index;
