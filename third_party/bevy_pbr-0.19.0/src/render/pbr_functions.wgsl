...
fn apply_pbr_lighting(
    ...
    for (var i: u32 = clusterable_object_index_ranges.first_point_light_index_offset;
        ...
        if shadow_capable && (!dominant_point_light_found || contribution_score > dominant_point_light_score) {
            dominant_point_light_found = true;
    ...
    if dominant_point_light_found {
...
#ifdef CONTACT_SHADOWS
#ifdef DEPTH_PREPASS
        if contact_shadow_enabled && dominant_shadow > 0.0 &&
                (view_bindings::clustered_lights.data[dominant_point_light_id].flags &
                    mesh_view_types::POINT_LIGHT_FLAGS_CONTACT_SHADOWS_ENABLED_BIT) != 0u {
            let L = normalize(
                view_bindings::clustered_lights.data[dominant_point_light_id].position_radius.xyz -
                in.world_position.xyz,
            );
            dominant_shadow *= calculate_contact_shadow(
                in.world_position.xyz,
                in.frag_coord.xy,
                L,
        ...
        let removed_fraction = 1.0 - dominant_shadow;
        direct_light -= dominant_point_light_contrib * removed_fraction;
#ifdef STANDARD_MATERIAL_DIFFUSE_TRANSMISSION
        if ((in.flags & MESH_FLAGS_TRANSMITTED_SHADOW_RECEIVER_BIT) != 0u) {
            transmitted_light -= dominant_point_transmitted_contrib * removed_fraction;
        }
#endif
    }
