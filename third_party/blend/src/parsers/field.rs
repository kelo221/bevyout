...
/// Blender represents a pointer to a fixed-size C array as `(*name)[N]`.
/// That is one pointer in the containing struct; the pointed-to array lives in
/// a separate DATA block. Treat it as an ordinary pointer so the runtime can
/// resolve and read the complete block.
fn pointer_to_array(input: &str) -> Result<(&str, FieldInfo)> {
    let (input, name) = delimited(tag("(*"), take_until(")"), tag(")"))(input)?;
    let (input, _) = array_dimensions(input)?;
    Ok((
        input,
        (
            name,
...
pub fn parse_field(input: &str) -> Result<(&str, FieldInfo)> {
    alt((pointer_to_array, fn_pointer, pointer, value))(input)
}
