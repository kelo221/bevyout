...
[Source](https://docs.rs/bevy_ptr/0.19.0/x86_64-unknown-linux-gnu/src/bevy_ptr/lib.rs.html#1-1549)
...
They’re also _the_ definitive source of memory safety bugs: you can dereference a invalid (null) pointer, access a pointer after the underlying memory has been freed, and even ignore type safety and misread or mutate the underlying memory improperly.
...
Correctly and safety converting a pointer into a valid borrow is at the core of all `unsafe` code in Rust. Looking at the documentation for \[`(*const T)::as_ref`\], a pointer must satisfy _all_ of the following conditions:
...
`&UnsafeCell<T>` is the first step away from safety. `UnsafeCell` is the _only_ way to get a mutable borrow from an immutable one in the language, so it’s the base primitive for all interior mutability in the language: `Cell<T>`, `RefCell<T>`, `Mutex<T>`, `RwLock<T>`, etc. are all built on top of `UnsafeCell<T>`. To safety convert `&UnsafeCell<T>` into a `&T` or `&mut T`, the caller must guarantee that all simultaneous access follow Rust’s aliasing rules.
...
`bevy_ptr` does not support these types right now, but may support [polyfills](https://en.wikipedia.org/wiki/Polyfill_\(programming\)) for these pointer types if the need arises.
...
`MovingPtr<'a, T>` is like a lifetimed-`Box<T>` or a typed `OwningPtr<'a>` made for cheaply moving potentially large values around in memory. It’s a pointer that owns the value it points to but does not own the allocation. If dropped, it will drop the value it points to, just as if you dropped a value of the inner type but won’t deallocate the allocation where the value lived in. It provides a number of methods for moving the value into another location in memory, including options for partial or deconstructive moves.

## Macros

[deconstruct\_moving\_ptr](macro.deconstruct_moving_ptr.html "macro bevy::ecs::ptr::deconstruct_moving_ptr")

Deconstructs a [`MovingPtr`](struct.MovingPtr.html "struct bevy::ecs::ptr::MovingPtr") into its individual fields.

[move\_as\_ptr](macro.move_as_ptr.html "macro bevy::ecs::ptr::move_as_ptr")
...
[Aligned](struct.Aligned.html "struct bevy::ecs::ptr::Aligned")

Used as a type argument to [`Ptr`](struct.Ptr.html "struct bevy::ecs::ptr::Ptr"), [`PtrMut`](struct.PtrMut.html "struct bevy::ecs::ptr::PtrMut"), [`OwningPtr`](struct.OwningPtr.html "struct bevy::ecs::ptr::OwningPtr"), and [`MovingPtr`](struct.MovingPtr.html "struct bevy::ecs::ptr::MovingPtr") to specify that the pointer is guaranteed to be [aligned](https://doc.rust-lang.org/std/ptr/index.html#alignment).

[ConstNonNull](struct.ConstNonNull.html "struct bevy::ecs::ptr::ConstNonNull")
...
[MovingPtr](struct.MovingPtr.html "struct bevy::ecs::ptr::MovingPtr")
...
[OwningPtr](struct.OwningPtr.html "struct bevy::ecs::ptr::OwningPtr")

Type-erased [`Box`](https://doc.rust-lang.org/std/boxed/struct.Box.html)\-like pointer to some unknown type chosen when constructing this type.

[Ptr](struct.Ptr.html "struct bevy::ecs::ptr::Ptr")

Type-erased borrow of some unknown type chosen when constructing this type.

[PtrMut](struct.PtrMut.html "struct bevy::ecs::ptr::PtrMut")

Type-erased mutable borrow of some unknown type chosen when constructing this type.

[ThinSlicePtr](struct.ThinSlicePtr.html "struct bevy::ecs::ptr::ThinSlicePtr")

Conceptually equivalent to `&'a [T]` but with length information cut out for performance reasons.

[Unaligned](struct.Unaligned.html "struct bevy::ecs::ptr::Unaligned")

Used as a type argument to [`Ptr`](struct.Ptr.html "struct bevy::ecs::ptr::Ptr"), [`PtrMut`](struct.PtrMut.html "struct bevy::ecs::ptr::PtrMut"), [`OwningPtr`](struct.OwningPtr.html "struct bevy::ecs::ptr::OwningPtr"), and [`MovingPtr`](struct.MovingPtr.html "struct bevy::ecs::ptr::MovingPtr") to specify that the pointer may not [aligned](https://doc.rust-lang.org/std/ptr/index.html#alignment).

## Traits

[IsAligned](trait.IsAligned.html "trait bevy::ecs::ptr::IsAligned")
...
[UnsafeCellDeref](trait.UnsafeCellDeref.html "trait bevy::ecs::ptr::UnsafeCellDeref")
