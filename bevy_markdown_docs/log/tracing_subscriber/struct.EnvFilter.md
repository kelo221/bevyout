...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#199)
...
`EnvFilter` implements both the [`Layer`](#impl-Layer%3CS%3E) and [`Filter`](#impl-Filter%3CS%3E) traits, so it may be used for both [global filtering](layer/index.html#global-filtering "mod bevy::log::tracing_subscriber::layer") and [per-layer filtering](layer/index.html#per-layer-filtering "mod bevy::log::tracing_subscriber::layer"), respectively. See [the documentation on filtering with `Layer`s](layer/index.html#filtering-with-layers "mod bevy::log::tracing_subscriber::layer") for details.

The [`Targets`](filter/struct.Targets.html "struct bevy::log::tracing_subscriber::filter::Targets") type implements a similar form of filtering, but without the ability to dynamically enable events based on the current span context, and without filtering on field values. When these features are not required, [`Targets`](filter/struct.Targets.html "struct bevy::log::tracing_subscriber::filter::Targets") provides a lighter-weight alternative to [`EnvFilter`](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter").

## Directives

A filter consists of one or more comma-separated directives which match on [`Span`](https://docs.rs/tracing-core/0.1.36/x86_64-unknown-linux-gnu/tracing_core/span/index.html "mod tracing_core::span")s and [`Event`](../tracing/struct.Event.html "struct bevy::log::tracing::Event")s. Each directive may have a corresponding maximum verbosity [`level`](../struct.Level.html "struct bevy::log::Level") which enables (e.g., _selects for_) spans and events that match. Like `log`, `tracing` considers less exclusive levels (like `trace` or `info`) to be more verbose than more exclusive levels (like `error` or `warn`).

The directive syntax is similar to that of [`env_logger`](https://crates.io/crates/env_logger)’s. At a high level, the syntax for directives consists of several parts:
...
*   `target` matches the event or span’s target. In general, this is the module path and/or crate name. Examples of targets `h2`, `tokio::net`, or `tide::server`. For more information on targets, please refer to [`Metadata`](../tracing/struct.Metadata.html "struct bevy::log::tracing::Metadata")’s documentation.
*   `span` matches on the span’s name. If a `span` directive is provided alongside a `target`, the `span` directive will match on spans _within_ the `target`.
*   `field` matches on [fields](../tracing/field/struct.Field.html "struct bevy::log::tracing::field::Field") within spans. Field names can also be supplied without a `value` and will match on any [`Span`](https://docs.rs/tracing-core/0.1.36/x86_64-unknown-linux-gnu/tracing_core/span/index.html "mod tracing_core::span") or [`Event`](../tracing/struct.Event.html "struct bevy::log::tracing::Event") that has a field with that name. For example: `[span{field=\"value\"}]=debug`, `[{field}]=trace`.
*   `value` matches on the value of a span’s field. If a value is a numeric literal or a bool, it will match _only_ on that value. Otherwise, this filter matches the [`std::fmt::Debug`](https://doc.rust-lang.org/nightly/core/fmt/trait.Debug.html "trait core::fmt::Debug") output from the value.
*   `level` sets a maximum verbosity level accepted by this directive.

When a field value directive (`[{<FIELD NAME>=<FIELD_VALUE>}]=...`) matches a value’s [`std::fmt::Debug`](https://doc.rust-lang.org/nightly/core/fmt/trait.Debug.html "trait core::fmt::Debug") output (i.e., the field value in the directive is not a `bool`, `i64`, `u64`, or `f64` literal), the matched pattern may be interpreted as either a regular expression or as the precise expected output of the field’s [`std::fmt::Debug`](https://doc.rust-lang.org/nightly/core/fmt/trait.Debug.html "trait core::fmt::Debug") implementation. By default, these filters are interpreted as regular expressions, but this can be disabled using the [`Builder::with_regex`](filter/struct.Builder.html#method.with_regex "method bevy::log::tracing_subscriber::filter::Builder::with_regex") builder method to use precise matching instead.

When field value filters are interpreted as regular expressions, the [`regex` crate’s regular expression syntax](https://docs.rs/regex/1.11.1/regex/#syntax) is supported.

**Note**: When filters are constructed from potentially untrusted inputs, [disabling regular expression matching](filter/struct.Builder.html#method.with_regex "method bevy::log::tracing_subscriber::filter::Builder::with_regex") is strongly recommended.
...
*   Any portion of the directive can be omitted.
    *   The sole exception are the `field` and `value` directives. If a `value` is provided, a `field` must _also_ be provided. However, the converse does not hold, as fields can be matched without a value.
*   If only a level is provided, it will set the maximum level for all `Span`s and `Event`s that are not enabled by other filters.
...
*   `warn,tokio::net=info` will enable all spans and events that:
    *   are at the level `warn` or above, _or_
...
Parsing an `EnvFilter` from the [default environment variable](struct.EnvFilter.html#method.from_default_env "associated function bevy::log::tracing_subscriber::EnvFilter::from_default_env") (`RUST_LOG`):
...
Parsing an `EnvFilter` [from a user-provided environment variable](struct.EnvFilter.html#method.from_env "associated function bevy::log::tracing_subscriber::EnvFilter::from_env"):
...
Using `EnvFilter` as a [per-layer filter](layer/index.html#per-layer-filtering "mod bevy::log::tracing_subscriber::layer") to filter only a single [`Layer`](trait.Layer.html "trait bevy::log::tracing_subscriber::Layer"):
...
An `EnvFilter` is be constructed by parsing a string containing one or more directives. The [`EnvFilter::new`](struct.EnvFilter.html#method.new "associated function bevy::log::tracing_subscriber::EnvFilter::new") constructor parses an `EnvFilter` from a string, ignoring any invalid directives, while [`EnvFilter::try_new`](struct.EnvFilter.html#method.try_new "associated function bevy::log::tracing_subscriber::EnvFilter::try_new") returns an error if invalid directives are encountered. Similarly, the [`EnvFilter::from_env`](struct.EnvFilter.html#method.from_env "associated function bevy::log::tracing_subscriber::EnvFilter::from_env") and [`EnvFilter::try_from_env`](struct.EnvFilter.html#method.try_from_env "associated function bevy::log::tracing_subscriber::EnvFilter::try_from_env") constructors parse an `EnvFilter` from the value of the provided environment variable, with lossy and strict validation, respectively.

A [builder](struct.EnvFilter.html#method.builder "associated function bevy::log::tracing_subscriber::EnvFilter::builder") interface is available to set additional configuration options prior to parsing an `EnvFilter`. See the [`Builder` type’s documentation](filter/struct.Builder.html "struct bevy::log::tracing_subscriber::filter::Builder") for details on the options that can be configured using the builder.

## Implementations

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#243)

### impl [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#249)

#### pub const [DEFAULT\_ENV](#associatedconstant.DEFAULT_ENV): &'static [str](https://doc.rust-lang.org/nightly/std/primitive.str.html) = "RUST\_LOG"

`RUST_LOG` is the default environment variable used by [`EnvFilter::from_default_env`](struct.EnvFilter.html#method.from_default_env "associated function bevy::log::tracing_subscriber::EnvFilter::from_default_env") and [`EnvFilter::try_from_default_env`](struct.EnvFilter.html#method.try_from_default_env "associated function bevy::log::tracing_subscriber::EnvFilter::try_from_default_env").

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#262)

#### pub fn [builder](#method.builder)() -> [Builder](filter/struct.Builder.html "struct bevy::log::tracing_subscriber::filter::Builder")

Returns a [builder](https://rust-unofficial.github.io/patterns/patterns/creational/builder.html) that can be used to configure a new [`EnvFilter`](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter") instance.

The [`Builder`](filter/struct.Builder.html "struct bevy::log::tracing_subscriber::filter::Builder") type is used to set additional configurations, such as [whether regular expressions are enabled](filter/struct.Builder.html#method.with_regex "method bevy::log::tracing_subscriber::filter::Builder::with_regex") or [the default directive](filter/struct.Builder.html#method.with_default_directive "method bevy::log::tracing_subscriber::filter::Builder::with_default_directive") before parsing an [`EnvFilter`](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter") from a string or environment variable.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#289)

#### pub fn [from\_default\_env](#method.from_default_env)() -> [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

Returns a new `EnvFilter` from the value of the `RUST_LOG` environment variable, ignoring any invalid filter directives.

If the environment variable is empty or not set, or if it contains only invalid directives, a default directive enabling the [`ERROR`](../struct.Level.html#associatedconstant.ERROR "associated constant bevy::log::Level::ERROR") level is added.

To set additional configuration options prior to parsing the filter, use the [`Builder`](filter/struct.Builder.html "struct bevy::log::tracing_subscriber::filter::Builder") type instead.
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#320)

#### pub fn [from\_env](#method.from_env)<A>(env: A) -> [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

where A: [AsRef](https://doc.rust-lang.org/nightly/core/convert/trait.AsRef.html "trait core::convert::AsRef")<[str](https://doc.rust-lang.org/nightly/std/primitive.str.html)\>,
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#350)

#### pub fn [new](#method.new)<S>(directives: S) -> [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

where S: [AsRef](https://doc.rust-lang.org/nightly/core/convert/trait.AsRef.html "trait core::convert::AsRef")<[str](https://doc.rust-lang.org/nightly/std/primitive.str.html)\>,

Returns a new `EnvFilter` from the directives in the given string, ignoring any that are invalid.

If the string is empty or contains only invalid directives, a default directive enabling the [`ERROR`](../struct.Level.html#associatedconstant.ERROR "associated constant bevy::log::Level::ERROR") level is added.
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#379)

#### pub fn [try\_new](#method.try_new)<S>(dirs: S) -> [Result](https://doc.rust-lang.org/nightly/core/result/enum.Result.html "enum core::result::Result")<[EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter"), [ParseError](filter/struct.ParseError.html "struct bevy::log::tracing_subscriber::filter::ParseError")\>

where S: [AsRef](https://doc.rust-lang.org/nightly/core/convert/trait.AsRef.html "trait core::convert::AsRef")<[str](https://doc.rust-lang.org/nightly/std/primitive.str.html)\>,

Returns a new `EnvFilter` from the directives in the given string, or an error if any are invalid.

If the string is empty, a default directive enabling the [`ERROR`](../struct.Level.html#associatedconstant.ERROR "associated constant bevy::log::Level::ERROR") level is added.
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#399)

#### pub fn [try\_from\_default\_env](#method.try_from_default_env)() -> [Result](https://doc.rust-lang.org/nightly/core/result/enum.Result.html "enum core::result::Result")<[EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter"), [FromEnvError](filter/struct.FromEnvError.html "struct bevy::log::tracing_subscriber::filter::FromEnvError")\>
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#420)

#### pub fn [try\_from\_env](#method.try_from_env)<A>(env: A) -> [Result](https://doc.rust-lang.org/nightly/core/result/enum.Result.html "enum core::result::Result")<[EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter"), [FromEnvError](filter/struct.FromEnvError.html "struct bevy::log::tracing_subscriber::filter::FromEnvError")\>
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#477)

#### pub fn [add\_directive](#method.add_directive)(self, directive: [Directive](filter/struct.Directive.html "struct bevy::log::tracing_subscriber::filter::Directive")) -> [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")
...
Filters may be created from [`LevelFilter`](../tracing/level_filters/struct.LevelFilter.html "struct bevy::log::tracing::level_filters::LevelFilter") or [`Level`](../struct.Level.html "struct bevy::log::Level"), which will enable all traces at or below a certain verbosity level, or parsed from a string specifying a directive.
...
From [`LevelFilter`](../tracing/level_filters/struct.LevelFilter.html "struct bevy::log::tracing::level_filters::LevelFilter"):
...
Or from [`Level`](../struct.Level.html "struct bevy::log::Level"):
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#498)

#### pub fn [enabled](#method.enabled)<S>(&self, metadata: &[Metadata](../tracing/struct.Metadata.html "struct bevy::log::tracing::Metadata")<'\_>, \_: [Context](layer/struct.Context.html "struct bevy::log::tracing_subscriber::layer::Context")<'\_, S>) -> [bool](https://doc.rust-lang.org/nightly/std/primitive.bool.html)

Returns `true` if this `EnvFilter` would enable the provided `metadata` in the current context.

This is equivalent to calling the [`Layer::enabled`](trait.Layer.html#method.enabled "method bevy::log::tracing_subscriber::Layer::enabled") or [`Filter::enabled`](layer/trait.Filter.html#tymethod.enabled "method bevy::log::tracing_subscriber::layer::Filter::enabled") methods on `EnvFilter`’s implementations of those traits, but it does not require the trait to be in scope.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#550)

#### pub fn [max\_level\_hint](#method.max_level_hint)(&self) -> [Option](https://doc.rust-lang.org/nightly/core/option/enum.Option.html "enum core::option::Option")<[LevelFilter](../tracing/level_filters/struct.LevelFilter.html "struct bevy::log::tracing::level_filters::LevelFilter")\>

Returns an optional hint of the highest [verbosity level](../struct.Level.html "struct bevy::log::Level") that this `EnvFilter` will enable.

This is equivalent to calling the [`Layer::max_level_hint`](trait.Layer.html#method.max_level_hint "method bevy::log::tracing_subscriber::Layer::max_level_hint") or [`Filter::max_level_hint`](layer/trait.Filter.html#method.max_level_hint "method bevy::log::tracing_subscriber::layer::Filter::max_level_hint") methods on `EnvFilter`’s implementations of those traits, but it does not require the trait to be in scope.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#568)

#### pub fn [on\_new\_span](#method.on_new_span)<S>(&self, attrs: &[Attributes](../tracing/span/struct.Attributes.html "struct bevy::log::tracing::span::Attributes")<'\_>, id: &[Id](../tracing/struct.Id.html "struct bevy::log::tracing::Id"), \_: [Context](layer/struct.Context.html "struct bevy::log::tracing_subscriber::layer::Context")<'\_, S>)

Informs the filter that a new span was created.

This is equivalent to calling the [`Layer::on_new_span`](trait.Layer.html#method.on_new_span "method bevy::log::tracing_subscriber::Layer::on_new_span") or [`Filter::on_new_span`](layer/trait.Filter.html#method.on_new_span "method bevy::log::tracing_subscriber::layer::Filter::on_new_span") methods on `EnvFilter`’s implementations of those traits, but it does not require the trait to be in scope.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#581)

#### pub fn [on\_enter](#method.on_enter)<S>(&self, id: &[Id](../tracing/struct.Id.html "struct bevy::log::tracing::Id"), \_: [Context](layer/struct.Context.html "struct bevy::log::tracing_subscriber::layer::Context")<'\_, S>)

Informs the filter that the span with the provided `id` was entered.

This is equivalent to calling the [`Layer::on_enter`](trait.Layer.html#method.on_enter "method bevy::log::tracing_subscriber::Layer::on_enter") or [`Filter::on_enter`](layer/trait.Filter.html#method.on_enter "method bevy::log::tracing_subscriber::layer::Filter::on_enter") methods on `EnvFilter`’s implementations of those traits, but it does not require the trait to be in scope.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#595)

#### pub fn [on\_exit](#method.on_exit)<S>(&self, id: &[Id](../tracing/struct.Id.html "struct bevy::log::tracing::Id"), \_: [Context](layer/struct.Context.html "struct bevy::log::tracing_subscriber::layer::Context")<'\_, S>)

Informs the filter that the span with the provided `id` was exited.

This is equivalent to calling the [`Layer::on_exit`](trait.Layer.html#method.on_exit "method bevy::log::tracing_subscriber::Layer::on_exit") or [`Filter::on_exit`](layer/trait.Filter.html#method.on_exit "method bevy::log::tracing_subscriber::layer::Filter::on_exit") methods on `EnvFilter`’s implementations of those traits, but it does not require the trait to be in scope.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#606)

#### pub fn [on\_close](#method.on_close)<S>(&self, id: [Id](../tracing/struct.Id.html "struct bevy::log::tracing::Id"), \_: [Context](layer/struct.Context.html "struct bevy::log::tracing_subscriber::layer::Context")<'\_, S>)

Informs the filter that the span with the provided `id` was closed.

This is equivalent to calling the [`Layer::on_close`](trait.Layer.html#method.on_close "method bevy::log::tracing_subscriber::Layer::on_close") or [`Filter::on_close`](layer/trait.Filter.html#method.on_close "method bevy::log::tracing_subscriber::layer::Filter::on_close") methods on `EnvFilter`’s implementations of those traits, but it does not require the trait to be in scope.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#622)

#### pub fn [on\_record](#method.on_record)<S>(&self, id: &[Id](../tracing/struct.Id.html "struct bevy::log::tracing::Id"), values: &[Record](../tracing/span/struct.Record.html "struct bevy::log::tracing::span::Record")<'\_>, \_: [Context](layer/struct.Context.html "struct bevy::log::tracing_subscriber::layer::Context")<'\_, S>)

Informs the filter that the span with the provided `id` recorded the provided field `values`.

This is equivalent to calling the [`Layer::on_record`](trait.Layer.html#method.on_record "method bevy::log::tracing_subscriber::Layer::on_record") or [`Filter::on_record`](layer/trait.Filter.html#method.on_record "method bevy::log::tracing_subscriber::layer::Filter::on_record") methods on `EnvFilter`’s implementations of those traits, but it does not require the trait to be in scope

## Trait Implementations

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#213)

### impl [Clone](https://doc.rust-lang.org/nightly/core/clone/trait.Clone.html "trait core::clone::Clone") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

Creates an [`EnvFilter`](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter") with the same directives as `self`.

This does _not_ clone any of the dynamic state that [`EnvFilter`](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter") acquires while attached to a subscriber.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#214)

#### fn [clone](https://doc.rust-lang.org/nightly/core/clone/trait.Clone.html#tymethod.clone)(&self) -> [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#198)

### impl [Debug](https://doc.rust-lang.org/nightly/core/fmt/trait.Debug.html "trait core::fmt::Debug") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#768)

### impl [Default](https://doc.rust-lang.org/nightly/core/default/trait.Default.html "trait core::default::Default") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#769)

#### fn [default](https://doc.rust-lang.org/nightly/core/default/trait.Default.html#tymethod.default)() -> [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

Returns the “default value” for a type. [Read more](https://doc.rust-lang.org/nightly/core/default/trait.Default.html#tymethod.default)

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#774)

### impl [Display](https://doc.rust-lang.org/nightly/core/fmt/trait.Display.html "trait core::fmt::Display") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#775)
...
### impl<S> [Filter](layer/trait.Filter.html "trait bevy::log::tracing_subscriber::layer::Filter")<S> for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

Available on **crate features `registry` and `std`** only.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#710)

#### fn [enabled](layer/trait.Filter.html#tymethod.enabled)(&self, meta: &[Metadata](../tracing/struct.Metadata.html "struct bevy::log::tracing::Metadata")<'\_>, ctx: &[Context](layer/struct.Context.html "struct bevy::log::tracing_subscriber::layer::Context")<'\_, S>) -> [bool](https://doc.rust-lang.org/nightly/std/primitive.bool.html)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#715)

#### fn [callsite\_enabled](layer/trait.Filter.html#method.callsite_enabled)(&self, meta: &'static [Metadata](../tracing/struct.Metadata.html "struct bevy::log::tracing::Metadata")<'static>) -> [Interest](../tracing/subscriber/struct.Interest.html "struct bevy::log::tracing::subscriber::Interest")
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#720)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#725)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#730)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#735)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#740)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#745)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#759-761)

### impl<S> [From](https://doc.rust-lang.org/nightly/core/convert/trait.From.html "trait core::convert::From")<S> for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

where S: [AsRef](https://doc.rust-lang.org/nightly/core/convert/trait.AsRef.html "trait core::convert::AsRef")<[str](https://doc.rust-lang.org/nightly/std/primitive.str.html)\>,

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#763)

#### fn [from](https://doc.rust-lang.org/nightly/core/convert/trait.From.html#tymethod.from)(s: S) -> [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

Converts to this type from the input type.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#751)

### impl [FromStr](https://doc.rust-lang.org/nightly/core/str/traits/trait.FromStr.html "trait core::str::traits::FromStr") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#752)

#### type [Err](https://doc.rust-lang.org/nightly/core/str/traits/trait.FromStr.html#associatedtype.Err) = [ParseError](filter/struct.ParseError.html "struct bevy::log::tracing_subscriber::filter::ParseError")

The associated error which can be returned from parsing.

[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#754)

#### fn [from\_str](https://doc.rust-lang.org/nightly/core/str/traits/trait.FromStr.html#tymethod.from_str)(spec: &[str](https://doc.rust-lang.org/nightly/std/primitive.str.html)) -> [Result](https://doc.rust-lang.org/nightly/core/result/enum.Result.html "enum core::result::Result")<[EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter"), <[EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter") as [FromStr](https://doc.rust-lang.org/nightly/core/str/traits/trait.FromStr.html "trait core::str::traits::FromStr")\>::[Err](https://doc.rust-lang.org/nightly/core/str/traits/trait.FromStr.html#associatedtype.Err "type core::str::traits::FromStr::Err")\>
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#664)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#674)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#679)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#684)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#689)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#694)
...
[Source](https://docs.rs/tracing-subscriber/0.3.23/x86_64-unknown-linux-gnu/src/tracing_subscriber/filter/env/mod.rs.html#699)
...
### impl ![Freeze](https://doc.rust-lang.org/nightly/core/marker/trait.Freeze.html "trait core::marker::Freeze") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

### impl [RefUnwindSafe](https://doc.rust-lang.org/nightly/core/panic/unwind_safe/trait.RefUnwindSafe.html "trait core::panic::unwind_safe::RefUnwindSafe") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

### impl [Send](https://doc.rust-lang.org/nightly/core/marker/trait.Send.html "trait core::marker::Send") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

### impl [Sync](https://doc.rust-lang.org/nightly/core/marker/trait.Sync.html "trait core::marker::Sync") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

### impl [Unpin](https://doc.rust-lang.org/nightly/core/marker/trait.Unpin.html "trait core::marker::Unpin") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

### impl [UnsafeUnpin](https://doc.rust-lang.org/nightly/core/marker/trait.UnsafeUnpin.html "trait core::marker::UnsafeUnpin") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")

### impl [UnwindSafe](https://doc.rust-lang.org/nightly/core/panic/unwind_safe/trait.UnwindSafe.html "trait core::panic::unwind_safe::UnwindSafe") for [EnvFilter](struct.EnvFilter.html "struct bevy::log::tracing_subscriber::EnvFilter")
...
### impl<F, S> [FilterExt](filter/trait.FilterExt.html "trait bevy::log::tracing_subscriber::filter::FilterExt")<S> for F
...
#### fn [and](filter/trait.FilterExt.html#method.and)<B>(self, other: B) -> [And](filter/combinator/struct.And.html "struct bevy::log::tracing_subscriber::filter::combinator::And")<Self, B, S>
...
Combines this [`Filter`](layer/trait.Filter.html "trait bevy::log::tracing_subscriber::layer::Filter") with another [`Filter`](layer/trait.Filter.html "trait bevy::log::tracing_subscriber::layer::Filter") s so that spans and events are enabled if and only if _both_ filters return `true`. [Read more](filter/trait.FilterExt.html#method.and)
...
#### fn [or](filter/trait.FilterExt.html#method.or)<B>(self, other: B) -> [Or](filter/combinator/struct.Or.html "struct bevy::log::tracing_subscriber::filter::combinator::Or")<Self, B, S>

where Self: [Sized](https://doc.rust-lang.org/nightly/core/marker/trait.Sized.html "trait core::marker::Sized"), B: [Filter](layer/trait.Filter.html "trait bevy::log::tracing_subscriber::layer::Filter")<S>,

Combines two [`Filter`](layer/trait.Filter.html "trait bevy::log::tracing_subscriber::layer::Filter")s so that spans and events are enabled if _either_ filter returns `true`. [Read more](filter/trait.FilterExt.html#method.or)
...
#### fn [not](filter/trait.FilterExt.html#method.not)(self) -> [Not](filter/combinator/struct.Not.html "struct bevy::log::tracing_subscriber::filter::combinator::Not")<Self, S>

where Self: [Sized](https://doc.rust-lang.org/nightly/core/marker/trait.Sized.html "trait core::marker::Sized"),

Inverts `self`, returning a filter that enables spans and events only if `self` would _not_ enable them. [Read more](filter/trait.FilterExt.html#method.not)
...
[Boxes](https://doc.rust-lang.org/nightly/alloc/boxed/index.html "mod alloc::boxed") `self`, erasing its concrete type. [Read more](filter/trait.FilterExt.html#method.boxed)
