...
fn hud_projection_lines(world: &mut World) -> Vec<String> {
...
fn hud_projection_is_empty_without_a_roster_resource() {
    let mut world = World::new();
    world.spawn((TestNavAgentMarker, GlobalTransform::default()));
...
fn hud_projection_orders_roster_agents_first_and_caps_the_tail() {
    ...
    world.resource_mut::<TestNavAgentState>().entities[0] = Some(late_roster_agent);
