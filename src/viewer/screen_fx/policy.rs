...
impl ScreenFxPolicy {
    ...
    pub fn sample(&self, definition: &ScreenFxDefinition, elapsed_ms: u64) -> ScreenFxValues {
        let sample_limit = if definition.duration_ms == 0 {
            u64::from(u32::MAX)
        } else {
            u64::from(definition.duration_ms)
        };
        let elapsed_ms = elapsed_ms.min(sample_limit) as u32;
    ...
    for curve in &mut definition.curves {
        curve.keyframes.retain(|keyframe| {
            keyframe.value.is_finite()
                && (definition.duration_ms == 0 || keyframe.time_ms <= definition.duration_ms)
        });
