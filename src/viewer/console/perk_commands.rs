...
//! persistent player progression resource. Definitions come from the
...
use super::stats::{PerkCatalog, PlayerProgression};
use super::stats_commands::player_perk_modifiers;
use super::*;
...
pub(super) fn add_perk(
    ...
    let def = perk_definition(world, form_id)?;
    {
        ...
        if let bevyout_core::perks::PerkEligibility::Blocked(reasons) =
            can_take_perk(&progression.stats, &def, &progression.perks)
        {
            return Err(ConsoleError::new(
                "perk_ineligible",
                format!(
    ...
    let rank = {
        ...
        let rank = progression.perks.rank(form_id) + 1;
        progression.perks.set_rank(form_id, rank);
        rank
    };
...
pub(super) fn remove_perk(
    ...
    let rank = {
        let mut progression = player_progression_mut(world)?;
        let owned = progression.perks.rank(form_id);
        if owned == 0 {
            return Err(ConsoleError::new(
                "perk_not_owned",
...
pub(super) fn has_perk(
    ...
    let def = perk_definition(world, form_id)?;
    let rank = player_progression(world)?.perks.rank(form_id);
    Ok(ConsoleCommandResult::new(
...
pub(super) fn show_perks(
    ...
    let catalog = world.resource::<PerkCatalog>().0.clone();
    let progression = player_progression(world)?;
    if eligible_flag {
        ...
        for def in catalog.values().filter(|def| def.playable) {
            let eligibility = can_take_perk(&progression.stats, def, &progression.perks);
            eligible.push(json!({
    ...
    for (form_id, rank) in progression.perks.0.iter() {
