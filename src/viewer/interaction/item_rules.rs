...
/// objects in FO3.
pub(crate) fn can_drop(base_form_id: u32, quest_item: bool) -> Result<(), TransferRejection> {
    if quest_item {
...
pub(crate) fn can_store(quest_item: bool) -> Result<(), TransferRejection> {
    if quest_item {
        Err(TransferRejection::QuestItem)
    } else {
        Ok(())
...
pub(crate) fn carried_weight(quest_item: bool, weight: Option<f32>) -> Option<f32> {
    if quest_item { None } else { weight }
}

/// F81.4: Take-vs-Steal for a reference with an optional `XOWN` owner.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum TakeClassification {
    Take,
...
pub(crate) fn classify_take(owner_form_id: Option<u32>) -> TakeClassification {
    match owner_form_id {
