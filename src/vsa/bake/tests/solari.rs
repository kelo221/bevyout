...
fn bake_shader_keeps_solari_scene_and_bevyout_bindings_separate() {
    ...
    assert!(shader.contains("@group(1) @binding(11) var<storage, read> environment_texels"));
    ...
    assert!(shader.contains("srgb_to_linear_rgb(vertex_color.xyz)"));
    assert!(shader.contains("alpha_materials"));
    assert!(shader.contains("hit_blocks_ray"));
    assert!(shader.contains("ray_visible"));
...
fn headless_solari_two_bounces_accumulate_authored_diffuse_transport() {
    use crate::vsa::bake::rust_scene::TransportMaterial;
    use bevy::math::Vec4;
    use bevyout_core::lighting::srgb_to_linear_rgb;

    let plane = |name: &str, z: f32, material: usize, transport_color: Vec4| ComposedPrimitive {
        name: name.into(),
        primitive_key: name.into(),
        reference_form_ids: vec![material as u32 + 1],
        material,
        positions: vec![
            Vec3::new(-1000.0, -1000.0, z),
            Vec3::new(1000.0, -1000.0, z),
            Vec3::new(0.0, 1000.0, z),
        ],
        normals: vec![Vec3::Z; 3],
        uvs: vec![[0.0, 0.0].into(), [1.0, 0.0].into(), [0.5, 1.0].into()],
        colors: vec![[1.0; 4].into(); 3],
    ...
    let primitives = vec![
        plane("receiver", 0.0, 0, Vec4::ONE),
        plane("secondary", 1.0, 1, Vec4::splat(0.5)),
        plane("tertiary", 2.0, 2, Vec4::splat(0.5)),
    ];
    ...
    let linear_albedo = srgb_to_linear_rgb([0.5; 3])[0] * srgb_to_linear_rgb([0.5; 3])[0] * 0.5;
    ...
    assert!((one - Vec3::splat(expected_one)).abs().max_element() <= 1.0e-3);
    assert!((two - Vec3::splat(expected_two)).abs().max_element() <= 1.0e-3);
...
fn headless_solari_constant_environment_matches_pi_irradiance() {
    ...
    assert!(
        (escape_value - Vec3::splat(expected_direct * 2.0))
            .abs()
            .max_element()
            <= 1.0e-3,
        "constant environment escape mismatch: actual={escape_value:?}, expected={}",
        expected_direct * 2.0
    );
}
